...
export async function parseIngredientTextWithAI(
  ...
  for (const token of tokens) {
    ...
    if (ing && !seen.has(ing.id)) {
      ...
      const fallback = makeFallbackIngredient(token);
      if (!seen.has(fallback.id)) {
        seen.add(fallback.id);
        ingredients.push(fallback);
        registerIngredient(fallback);
      }
    }
  }
...
export function analyseIngredients(
  ...
  if (profile) {
    ...
    for (const ing of ingredients) {
      ...
      // 2d. Skin-profile category fit
      if (!warned && profile.skinType === "dry" && ["Surfactant", "Active - Exfoliant", "Active - Anti-acne"].includes(ing.category)) {
        warnings.push({ type: "unsuitableForSkin", severity: "Low",
          title: `${ing.name} may feel drying`,
          message: `${ing.name} can be less comfortable for dry skin, especially when your barrier is already dehydrated.`,
          ingredientNames: [ing.name] });
        score = deduct(score, "Low"); warned = true;
      }

      if (!warned && (profile.conditions.includes("rosacea") || profile.conditions.includes("eczema")) &&
        ["Active - Exfoliant", "Active - Anti-aging", "Active - Brightening", "Active - Anti-acne"].includes(ing.category)) {
        warnings.push({ type: "unsuitableForSkin", severity: "Low",
          title: `${ing.name} may be too active`,
          message: `${ing.name} belongs to an active category that can be tricky for reactive skin profiles.`,
          ingredientNames: [ing.name] });
        score = deduct(score, "Low"); warned = true;
      }
